...
import time
config = Config(dataset_name='gears')
...
class gears_Dataset():
    ...
    def __init__(self, dataset_name):
        ...
        self.cls_lst = bs_utils.read_lines(config.gears_cls_lst_p)
        ...
        self.sym_cls_ids = config.gears_sym_cls_ids
    ...
    def get_item(self, item_name):
        ...
        try:
            ...
            '''with Image.open(os.path.join("./datasets/openDR/openDR_dataset/depth/{}.png".format(item_name) )) as di:
                ...
                dpt = dpt/65535
                dpt = dpt*3.0'''
            with Image.open(os.path.join("./datasets/gears/gears_dataset/mask/{}.png".format(item_name))) as li:
                labels = np.array(li)
                #cls_id_lst = np.unique(labels[labels.nonzero()])
            meta = scio.loadmat(os.path.join('./datasets/gears/gears_dataset/meta/', item_name+'-meta.mat'))
            #if item_name[:8] != 'data_syn' and int(item_name[5:9]) >= 60:
            #    K = config.intrinsic_matrix['openDR']
            #else:
            K = config.intrinsic_matrix['gears']
            ...
            for i, cls_id in enumerate(range(1,8)):
                ...
                ctr = bs_utils.get_ctr(i, ds_type='gears').copy()[:, None]
                ...
                kps = bs_utils.get_kps(
                    i, kp_type=kp_type, ds_type='gears'
                ).copy()
