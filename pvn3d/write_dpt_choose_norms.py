...
for i,_ in enumerate(listdir('./datasets/gears/gears_dataset/depth')):
#for i in range(2304, 3071):#2304):

    with Image.open('./datasets/gears/gears_dataset/depth/'+str(i)+'.png') as dpt_im:
        dpt = np.array(dpt_im)
        print(dpt.dtype)
        print(dpt.max())
    ...
    cld, choose = bs_utils.dpt_2_cld(dpt, 1, cfg.intrinsic_matrix['gears'])
    ...
    with open('./datasets/gears/gears_dataset/'+str(i)+'.npy', 'wb') as f:
